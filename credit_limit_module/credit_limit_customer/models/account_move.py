...
class SaleOrder(models.Model):
    ...
    applied_credit_limit = fields.Float(
        string='Limite de credito',
        compute='update_credit_limit',
        ...
        if self.partner_id:
            if self.partner_id.credit_limit and self.partner_id.active_limit:
                for s in self:
                    ...
                    if (
                        self.partner_id.block_SO == 0
                        and self.amount_total > self.applied_credit_limit
                        or self.partner_id.block_SO == 0
                        and self.partner_id.total_debt >= self.applied_credit_limit
                    ):
                        return {
                            'value': {},
                            'warning': {
                        ...
                        self.partner_id.block_SO
                        and self.amount_total > self.applied_credit_limit
                        or self.partner_id.block_SO
                        and self.partner_id.total_debt >= self.applied_credit_limit
                    ):
                        raise ValidationError(
                            _(
                                'No puede facturar \nEl cliente excede el limite de credito establecido: {}'.format(
                                    s.applied_credit_limit
                                )
                            )
                        )
